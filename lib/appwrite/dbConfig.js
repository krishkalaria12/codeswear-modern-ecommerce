...
export class Service{
    ...
    async getOrderByID(userid, orderid ,queries = [Query.equal("orderid", orderid), Query.equal("userid", userid)]){
        try {
            return await this.databases.listDocuments(
                conf.appwriteDatabaseId,
                conf.appwriteOrdersCollectionId,
            ...
            console.log("Appwrite serive :: getOrderbyId :: error", error);
            return false
        }
    }
    ...
    async createOrder({firstname,lastname,email,phoneno,city,country,flatno,postalCode ,state, orderid, userid, totalPrice, address}) {
        try {
            return await this.databases.createDocument(
                conf.appwriteDatabaseId,
                conf.appwriteOrdersCollectionId,
                ID.unique(),
                {firstname,lastname,email,phoneno,city,country,flatno,state,orderid,postalCode, userid, address, totalPrice}
            )
        } catch (error) {

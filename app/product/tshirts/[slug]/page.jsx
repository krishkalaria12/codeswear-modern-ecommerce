...
function Product({ params }) {
  ...
  const fetchProduct = async () => {
    ...
    const fetchProductSlug = await service.getPostBySlug(slug, conf.appwriteTshirtsCollectionId);
    ...
    const products = await service.getPostsByName(
      fetchProductSlug.documents[0].name, conf.appwriteTshirtsCollectionId
    );
  ...
  const refreshVariant = (newsize, newcolor) => {
    let url = `${process.env.NEXT_PUBLIC_HOST}/product/tshirts/${variants[newcolor][newsize]["slug"]}`;
    router.push(url);
  };
